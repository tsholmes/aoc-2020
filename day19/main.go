...
func part1() {
	...
	for _, line := range secs[0] {
		...
		num := ps[0]
		if ps[1][0] == '"' {
			rules[num] = rule{char: byte(ps[1][1])}
		} else {
			rps := strings.Split(ps[1], " | ")
			var sps [][]string
			for _, rp := range rps {
				sps = append(sps, strings.Split(rp, " "))
			}
			rules[num] = rule{parts: sps}
		}
	}
	type key struct {
		str  string
	...
	memo := map[key]bool{}
	var match func(str string, rule string) bool
	match = func(str string, rule string) bool {
		k := key{str, rule}
		if v, ok := memo[k]; ok {
			return v
		}
		...
		for _, ps := range r.parts {
			if len(ps) == 1 {
				...
				for i := 1; i < len(str); i++ {
					if match(str[:i], ps[0]) && match(str[i:], ps[1]) {
						res = true
						break
					}
				}
	...
	match = func(str string, rule string) bool {
		...
		for _, ps := range r.parts {
			if len(ps) == 1 {
				...
				for i := 1; i < len(str)-1 && !res; i++ {
					if !match(str[:i], ps[0]) {
						continue
					}
					for j := i + 1; j < len(str); j++ {
						if match(str[i:j], ps[1]) && match(str[j:], ps[2]) {
							res = true
							break
						}
					}
