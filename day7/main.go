...
func part1() {
	...
	for _, line := range aoc.Lines(input) {
		ps := strings.Split(line, " contain ")
		name := ps[0][:len(ps[0])-5]
		sizes[name] = map[string]int{}
		rules := strings.Split(ps[1][:len(ps[1])-1], ", ")
		for _, rule := range rules {
			pps := strings.SplitN(rule, " ", 2)
			var c int
			fmt.Sscanf(pps[0], "%d", &c)
			iname := pps[1]
			if iname[len(iname)-1] == 's' {
				iname = iname[:len(iname)-5]
			} else {
				iname = iname[:len(iname)-4]
			}
			sizes[name][iname] = c
		}
	}
...
func part2() {
	...
	memo := map[string]int{}
	var search func(name string) int
	search = func(name string) int {
		if v, ok := memo[name]; ok {
			return v
		}
		count := 0
		for n2, c := range sizes[name] {
			count += c * (1 + search(n2))
		}
		memo[name] = count
		return count
	...
	fmt.Println(search("shiny gold"))
}

const input = `vibrant purple bags contain 3 shiny lavender bags, 1 mirrored gray bag, 4 muted bronze bags.
