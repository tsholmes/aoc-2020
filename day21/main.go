...
func part1() {
	...
	for _, line := range lines {
		ps := strings.Split(line, " (contains ")
		ings := strings.Split(ps[0], " ")
		ags := strings.Split(ps[1][:len(ps[1])-1], ", ")
		recipes = append(recipes, recipe{ings, ags})
	}
	pagmap := map[string][]string{}
	for _, r := range recipes {
		for _, ag := range r.allergens {
			if _, ok := pagmap[ag]; !ok {
				pagmap[ag] = append([]string{}, r.ingredients...)
				continue
			}
			ex := map[string]bool{}
			for _, ing := range pagmap[ag] {
				ex[ing] = true
			}
			ni := []string{}
